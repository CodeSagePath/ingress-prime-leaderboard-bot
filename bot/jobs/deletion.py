...
from ..models import GroupMessage, GroupPrivacyMode, GroupSetting, PendingAction
...
async def _persist_pending_actions(
    chat_id: int | None, message_ids: list[int | None]
) -> None:
    if chat_id is None:
        return
...
async def _delete_messages(token: str, payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            "Unable to verify permissions in chat %s: %s", payload.get("chat_id"), exc
        )
    ...
    if not allowed:
        logger.info(
            "Skipping deletion because bot lacks permission in chat %s",
            payload.get("chat_id"),
        )
        return
...
def schedule_message_deletion(
    ...
    if queue is None or not token or chat_id is None:
        return
    ids = [
        value for value in (message_id, confirmation_message_id) if value is not None
    ]
    if not ids:
        return
    effective_delay = max(delay_seconds, 0)
    run_at = datetime.utcnow() + timedelta(seconds=effective_delay)
    payload = {
        "chat_id": chat_id,
        "message_id": message_id,
    ...
    try:
        ...
        logger.error("Failed to schedule deletion for chat %s: %s", chat_id, exc)
...
def delete_message_job(token: str, payload: dict[str, Any]) -> None:
    try:
        ...
        logger.exception(
            "Deletion job failed for chat %s: %s", payload.get("chat_id"), exc
        )


async def execute_pending_deletions(
    application, session_factory: async_sessionmaker
) -> None:
    """Execute any pending deletions that failed previously"""
    async with session_scope(session_factory) as session:
        result = await session.execute(
            select(PendingAction).where(PendingAction.executed.is_(False))
        )
        ...
        for action in pending:
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        "Failed to delete pending message %s in chat %s after retry: %s",
                        action.message_id,
                        action.chat_id,
                        exc,
                    )
                ...
                logger.error(
                    "Failed to delete pending message %s in chat %s: %s",
                    action.message_id,
                    action.chat_id,
                    exc,
                )
    ...
    async with session_scope(session_factory) as session:
        ...
        for setting in group_settings:
            ...
            threshold = datetime.now(timezone.utc) - timedelta(
                minutes=setting.retention_minutes
            )
            ...
            while True:
                ...
                for record in rows:
                    try:
                        ...
                        try:
                            ...
                            logger.error(
                                "Failed to delete message %s in chat %s after retry: %s",
                                record.message_id,
                                record.chat_id,
                                exc,
                            )
                ...
                # Delete processed records from da
                # tabase
                if deleted_ids:
                    await session.execute(
                        delete(GroupMessage).where(GroupMessage.id.in_(deleted_ids))
                    )
                    await session.flush()
